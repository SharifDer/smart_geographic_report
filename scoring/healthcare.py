def score_competitive(healthcare_data, weight_score):
    ...
    pharmacies_per_10k = healthcare_data['pharmacy'].get('pharmacies_per_10k_population', 0)
    ...
    distance_score = min(nearest_distance / 5000, 1.0)

    # Normalize saturation: assume 0 to 20 pharmacies per 10k population
    saturation_score = max(0, 1 - pharmacies_per_10k / 20)
...
def score_healthcare_ecosystem(healthcare_data, weight_score):
    def score_proximity(places):
        ...
        avg_distance_score = max(0, 1 - avg_distance / max_distance)
        count_score = min(len(within_3km), 10) / 10
